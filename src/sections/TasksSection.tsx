...
export default function TasksSection() {
  ...
  const tasksTick = useTasks();

  // React to selected day changes from dateStore (epoch-day -> local midnight ms)
  const selectedEpochDay = useDateStore((s: any) => s.selected);
  const anchorMs = dayMs(selectedEpochDay);

  const items = useMemo(() => (tasksOnDay(anchorMs) as Task[]) ?? [], [anchorMs, tasksTick]);
  ...
  const { active, completed } = useMemo(() => {
    ...
    return {
      ...
      completed: items
        .filter(isDone)
        .sort((x: any, y: any) => (y.completedAt ?? 0) - (x.completedAt ?? 0)),
    };
  }, [items]);
  ...
  useEffect(() => {
    setDateVal(toDateInputValue(anchorMs));
  }, [anchorMs]);
  ...
  const commitAdd = useCallback(() => {
    ...
    try {
      const dueMs = fromDateInputValue(dateVal); // already at local midnight
    ...
    <section>
      ...
      {active.length === 0 ? (
        ...
        <div style={{ display: "grid", gap: 8 }}>
          {active.map((t: any) => {
            const tri = t.tri ?? 0;
            const done = Boolean(t.done ?? tri === 1);
